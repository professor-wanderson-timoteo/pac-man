...
class Demo {
  ...
  nextAnimation() {
    this.step =
      this.step === DemoData.animations.length - 1 ? 1 : this.step + 1;
  ...
  initFrighten() {
    var speed =
        Data.getLevelData("ghostFrightSpeed") * DemoData.frighten.speedMult,
  ...
  drawScores(time) {
    this.scores.forEach((score, index) => {
      score.timer += time;
      score.size = Math.min(
        0.2 + Math.round((score.timer * 100) / DemoData.chase.scoreInc) / 100,
        1
      );
