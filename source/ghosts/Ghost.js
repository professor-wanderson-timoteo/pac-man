class Ghost {
   ...
   * @param {Canvas} canvas
   * @param {?number} dots
   * @param {boolean} inPen Whether the Ghost starts in the Pen or not
   */
  init(canvas, dots, inPen = false) {
    this.canvas = canvas;
    this.ctx = canvas.context;
    this.mode = "scatter";
    this.inPen = inPen;
    this.tile = Board.getGhostStartTile(this.inPen); // Define where the Ghost starts
    this.tileCenter = Board.getTileXYCenter(this.tile);
    this.turn = Board.getGhostStartTurn(this.inPen); // Define the initial turn direction
    this.center = false;
    this.dotsCount = dots || 0;
    this.scatter = { x: 0, y: 0 }; // Set default scatter position
    this.target = this.scatter; // Default target is scatter
  ...
  /**
   * Switches the mode of the Ghost
  ...
  switchMode(oldMode, newMode, blob) {
    if (!this.dontSwitch(oldMode)) {
      this.mode = newMode;
      this.target = this.getTarget(blob); // Update target based on new mode
      this.speed = this.getSpeed();
      if (!this.dontHalfTurn(oldMode)) {
        if (this.path === null) {
          this.turn = { x: this.dir.x * -1, y: this.dir.y * -1 }; // Reverse direction if no path
        } else {
          this.turn = { x: 1, y: 0 }; // Default turn if there's a path
        }
      }
    }
  }
  ...
  /**
   * Moves the Ghost
   * @param {number} speed Multiplier for the speed
   * @param {Blob} blob
   * @param {string} switchMode Mode to switch if necessary
   */
  move(speed, blob, switchMode) {
    ...
    this.moveFeet(); // Update feet position
    this.draw(); // Draw the ghost's current position
    return addToPen;
  }

  /**
   * Moves the Ghost according to the current path
   * @param {Blob} blob
  ...
  /**
   * Normal movement for the Ghost when not following a path
   * @param {Blob} blob
   */
  normalMove(blob) {
    this.newTile(blob);
    ...
    if (!this.center && this.passedCenter()) {
      if (this.turn) {
        this.makeTurn(); // Perform a turn when reaching an intersection
      }
      if (this.isNextIntersection()) {
        this.decideTurn(); // Decide which direction to take at the intersection
      }
      this.speed = this.getSpeed();
      this.center = true;
    }
  ...
  /**
   * Updates the Ghost's current tile and center
   * @param {Blob} blob
   */
  newTile(blob) {
  ...
  /**
   * Sets the path for the Ghost to follow
  ...
  /**
   * Checks if the Ghost is entering the Pen
   * @return {boolean}
   */
  isEnteringPen() {
    return (
      this.mode === "eyes" && Board.equalTiles(this.tile, Board.eyesTarget)
    );
  }
  ...
  /**
   * Gets the possible turns the Ghost can make
   * @return {Array} Turns the Ghost can make
   */
  getTurns() {
  ...
  /**
   * Decides the best target turn based on distance
   * @param {Array} turns The possible turns
   * @return {Object} The best turn
   */
  getTargetTurn(turns) {
    let tile = this.getNextTile(),
      best = 999999,
  ...
  /**
   * Determines if the Ghost has caught the Blob
   * @param {Object} blobTile
   * @return {string} "kill" or "die"
   */
  killOrDie(blobTile) {
    if (Board.equalTiles(this.tile, blobTile)) {
  ...
  /**
   * Determines if the Ghost should change its target
  ...
  /**
   * Checks if the Ghost can switch modes
   * @param {string} mode
   * @return {boolean}
   */
  dontSwitch(mode) {
    return (
      (Data.isFrighten(mode) && !Data.isFrighten(this.mode)) ||
      this.mode === "eyes"
    );
  }

  /**
   * Checks if the Ghost should avoid turning half-way
   * @param {string} mode
   * @return {boolean}
   */
  dontHalfTurn(mode) {
  ...
  /**
   * Returns the current speed based on the mode and tile
   * @return {number} Speed of the Ghost
   */
  getSpeed() {
    let speed = Data.getGhostSpeed(false); // Default ghost speed

    if (this.mode === "eyes") {
      speed = Data.eyesSpeed; // Eyes mode speed
    } else if (Data.isFrighten(this.mode)) {
      speed = Data.getLevelData("ghostFrightSpeed"); // Frightened mode speed
    } else if (Board.isTunnel(this.tile.x, this.tile.y)) {
      speed = Data.getLevelData("tunnelSpeed"); // Tunnel speed
    } else if (this.isElroy()) {
      speed = Data.getLevelData("elroySpeed" + this.elroyMode); // Special Elroy mode speed
    }
